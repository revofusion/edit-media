...
const available = `IO... yuv420p                3             12      8-8-8
...
const formatFlags = (flags) => {
    const formats = ['input', 'output', 'hardware', 'paletted', 'bitstream'];
    let flagObject = {};
...
let json = lines.map(line => {
    const [flags, key, nb_components, bits_per_pixel, bit_depths] = line.split(/\s+/);
    return {flags: formatFlags(flags), key, nb_components, bits_per_pixel, bit_depths};
});
...
fs.writeFileSync(`${__dirname}/../pix.ts`, `
...
export const pixelFormats: PixelFormat[] = ${JSON.stringify(json, null, 2)}
`)
