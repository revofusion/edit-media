...
const available = `D.VI.S 012v                 Uncompressed 4:2:2 10-bit
D.V.L. 4xm                  4X Movie
...
DEV.L. av1                  Alliance for Open Media AV1 (decoders: libaom-av1 av1 ) (encoders: libaom-av1 libsvtav1 )
...
D.AIL. amr_nb               AMR-NB (Adaptive Multi-Rate NarrowBand) (decoders: amrnb amr_nb_at )
...
const formatFlags = (flags, type) => {
  const formats = ['decoding', 'encoding', type, 'intraframe', 'lossy', 'lossless'];
  let flagObject = {};
  flags.split('').forEach((flag, index) => {
      flagObject[formats[index]] = flag !== '.';
  });
  return flagObject;
};
...
let json = lines
  .map(line => {
    const [flags, key, ...description] = line.split(/\s+/);
    let type;
    if (flags[2] === 'V') {
        type = 'video';
    } else if (flags[2] === 'A') {
...
fs.writeFileSync(`${__dirname}/../codecs.ts`, `
export interface Codec {
  ...
  export const codecs: Codec[] = ${JSON.stringify(json, null, 2)}
